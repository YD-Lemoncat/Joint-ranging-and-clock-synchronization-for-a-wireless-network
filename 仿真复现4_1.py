...
for K in tqdm(K_values):  # 使用 tqdm 进行循环迭代
        ...
        def construct_T(N, K):
            ...
            for i in range(N):
                for j in range(i + 1, N):
                    for k in range(K):
                        start_index = (i * (N - 1) - i * (i + 1) // 2 + j - 1) * K * 2 + k * 2
                        end_index = start_index + 2
                        time_interval = np.linspace(1, 100, K + 1)
                        T[start_index:end_index, i] = [time_interval[k], time_interval[k+1]]   # 设置发送时间标记
                        T[start_index:end_index, j] = [-time_interval[k], -time_interval[k+1]] # 为接收时间标记添加负号
            return T
        T = construct_T(N, K)

        # 构造 E1 矩阵(依据T的形状构造矩阵 E1)
        def construct_E1(T):
            # 获取 T 矩阵的形状
    ...
    # 计算 MSE
    true_clock_skew = clock_skew_range[1:]  # 真实的时钟漂移
    true_clock_offset = clock_offset_range[1:]  # 真实的时钟偏移
